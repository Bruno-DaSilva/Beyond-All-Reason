...
function gadget:GetInfo()
	return {
		name = "Benchmark",
		desc = "Benchmark tool: spawns units in controlled battles to stress-test performance",
...
if gadgetHandler:IsSyncedCode() then
	...
	local function ExecuteRemoveUnitDefName(unitdefname)
		local unitDefID = UnitDefNames[unitdefname].id
		if unitDefID then
			local wreckFeatureDefID, heapFeatureDefID
	...
	local function benchmark(words)
	...
	function gadget:RecvLuaMsg(msg, playerID)
		...
		local cmd = words[1]
		if cmd ~= "benchmark" then return end
	...
	local function benchmark(_, line, words, playerID, action)
